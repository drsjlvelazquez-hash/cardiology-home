...
// ─── Storage Helper (shared across all users) ────────────────────────────────
const DB = {
  async get(key) {
...
export default function App() {
  ...
  const [view, setView] = useState("patient"); // patient | pin | physician
  const t = L[lang];

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&family=Lora:ital,wght@0,400;0,600;0,700;1,400&display=swap');
      ...
      <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#e8f0fe 0%,#f0f6ff 60%,#fef9ee 100%)" }}>
        {/* App Header */}
        <div style={appS.header}>
          <div style={appS.headerInner}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <span style={{ fontSize:28, color:"#fca5a5", filter:"drop-shadow(0 0 6px rgba(252,165,165,0.5))" }}>♥</span>
              <div>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
              {view === "patient" && (
                <button style={appS.secondaryBtn} onClick={() => setView("pin")}>
                  🩺 {t.physicianAccess}
                </button>
              )}
              {view === "patient" && (
                <button style={appS.langBtn} onClick={() => setLang(l => l==="en"?"es":"en")}>{t.langBtn}</button>
              )}
            </div>
          </div>
          {/* Tips */}
        ...
        {/* Views */}
        {view === "patient" && <PatientPortal lang={lang} />}
        {view === "pin" && <PINGate onSuccess={() => setView("physician")} onBack={() => setView("patient")} />}
        {view === "physician" && <PhysicianDashboard onBack={() => setView("patient")} />}
      </div>
    </>
  );
}
...
const ps = {
  ...
  codeBanner: { maxWidth:1100, margin:"0 auto 16px", padding:"0 20px" },
